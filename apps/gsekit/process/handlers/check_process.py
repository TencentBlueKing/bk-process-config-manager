...
class ProcessCheckManager:
    ...
    def __init__(self, bk_biz_id):
        ...
        self.process_template_id__info_map = {}  # {process_template_id: template_info}
    ...
    def get_script_content(self):
        """获取脚本内容"""
        return GlobalSettings.get_config(key=GlobalSettings.KEYS.CHECK_PROC_SCRIPT, default=self.SCRIPT_CONTENT)
    ...
    def _fetch_process_templates(self):
        ...
        self.process_template_id__info_map = {pt["process_template_id"]: pt for pt in process_template_info_list}
    ...
    def _process_process_insts(self):
        ...
        process_info = Process.objects.filter(bk_biz_id=self.bk_biz_id).values("bk_process_id", "process_template_id")
        process_template_id_map = {inst["bk_process_id"]: inst["process_template_id"] for inst in process_info}
        ...
        for inst in process_insts:
            ...
            process_template_id = process_template_id_map[inst["bk_process_id"]]
            template_info = self.process_template_id__info_map.get(process_template_id)
    ...
    def _execute_scripts_and_parse(self):
        ...
        for os_type, host_ids in os_type_host_map.items():
            ...
            job_kwargs = {
                ...
                "script_content": base64.b64encode(self.get_script_content()[str(os_type)].encode("utf8")).decode(
                    "utf8"
                ),
    ...
    def _parse_ip_logs(self, script_task_logs):
        ...
        for log in script_task_logs:
            ...
            try:
                ...
                decoded_object, _ = decoder.raw_decode(json_str)

                # 存储进程信息
                for proc in decoded_object.get("proc", []):

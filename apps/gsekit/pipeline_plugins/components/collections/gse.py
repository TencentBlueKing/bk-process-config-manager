...
class GseOperateProcessService(JobTaskBaseService):
    """
    GSE Service 基类
    """
        ...
        for process_attr in process_attrs:
            ...
            if raise_exception:
                raise ProcessAttrIsNotConfiguredException(
                    process_name=process_info.get("bk_process_name"),
    ...
    def _execute(self, data, parent_data):
        ...
        return self.return_data(result=True, job_extra_data={"inst_id": inst_id})
    ...
    def _schedule(self, data, parent_data, callback_data=None):
        ...
        data.outputs.proc_op_status = error_code

        if error_code == GseDataErrorCode.SUCCESS:
            process_inst = ProcessInst.objects.get(bk_process_id=job_task.bk_process_id, local_inst_id=local_inst_id)
            ...
            extra_data = {"failed_reason": self.generate_proc_op_error_msg(error_code, error_msg)}
            return self.return_data(result=True, job_extra_data=extra_data, is_finished=True)
        return self.return_data(result=True)

    def inputs_format(self):
        return super().inputs_format() + [JobTaskBaseService.InputItem(name="op_type", key="op_type", required=True)]

    def outputs_format(self):
        return [
            # 当前原子进程操作状态码
            JobTaskBaseService.OutputItem(name="proc_op_status", key="proc_op_status", type="int"),
        ] + super().outputs_format()


class GseCheckProcessService(GseOperateProcessService):
    def _execute(self, data, parent_data):
        last_proc_op_status = data.get_one_of_inputs("last_proc_op_status")
        # 上一个原子进程操作成功，无需查询进程状态, 也无需更新job_task
        if last_proc_op_status == GseDataErrorCode.SUCCESS:
    ...
    def _schedule(self, data, parent_data, callback_data=None):
        ...
        if error_code == GseDataErrorCode.SUCCESS:
            ...
            # last_proc_op_status不为None时，该原子为失败操作查询进程状态，任务延续上一原子的失败状态
            if last_proc_op_status is None:
                return self.return_data(result=True, is_finished=True)
            else:
                return self.return_data(result=False)
            ...
            error_msg = task_result.get("error_msg")
            extra_data = {"failed_reason": self.generate_proc_op_error_msg(error_code, error_msg)}
            return self.return_data(result=False, job_extra_data=extra_data)
        # RUNNING 进入下一次轮询
        return self.return_data(result=True)
    ...
    def inputs_format(self):
        return super().inputs_format() + [
            ...
            JobTaskBaseService.InputItem(name="last_proc_op_status", key="last_proc_op_status", required=False)
        ]


class GseOperateProcessComponent(Component):
    name = "GseOperateProcessComponent"
    code = "gse_operate_process"
    bound_service = GseOperateProcessService


class GseCheckProcessComponent(Component):
    name = "GseCheckProcessComponent"
    code = "gse_check_process"
    bound_service = GseCheckProcessService

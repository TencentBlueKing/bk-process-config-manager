    ...
    <div class="config-file-template-distribute-container" v-bkloading="{ isLoading: fakeLoading }">
        <div class="distribute-above">
            ...
            <div class="form-container" v-if="curStep === 1 && isMultipleTemplates">
                ...
                <bk-select class="king-select" searchable multiple display-tag show-select-all
                    v-model="selectedTemplateIds" @change="handleSelectTemplate">
                    <template v-for="option in validTemplates">
                        <bk-option :key="option.config_template_id" :id="option.config_template_id" :name="option.template_name"></bk-option>
                    </template>
                </bk-select>
            </div>
            <!-- 第二步配置生成上方的按钮 -->
            <div class="buttons-above" v-show="curStep === 2">
                <bk-button style="margin-right: 10px;"
                    :loading="generateLoading"
                    :disabled="isTasking"
                    @click="handleGenerateAll">
                    {{ $t('全部重新生成') }}
                </bk-button>
                <bk-button :loading="generateFailureLoading"
        ...
        <div class="distribute-below" v-bkloading="{ isLoading: basicLoading, opacity: 1, zIndex: 0 }">
            ...
            <div class="instance-table-container">
                <template v-for="temp in selectedTemplates">
                    <InstanceTable :key="temp.config_template_id"
        ...
        methods: {
            ...
            async handleGenerateAll () {
                ...
                const promiseList = this.validTableRefs.map(vm => {
                    return vm.generateConfig({ isFirstStep: false })
                })
            ...
            async handleNext () {
                ...
                const promiseList = this.validTableRefs.map(vm => {
                    return vm.generateConfig({ isFirstStep: true })
                })
                const result = await Promise.all(promiseList)
                this.isTasking = false
                this.isAllGeneratedSuccess = result.every(Boolean)
            ...
            async handleDistribute () {
                ...
                if (result.length === 1) {
                    ...
                    const ids = result.map(item => item.jobId).filter(Boolean).join(',')
